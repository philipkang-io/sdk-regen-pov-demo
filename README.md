...
```bash
npm install payments-api-sdk
```

or

```bash
yarn add payments-api-sdk
```
...
The PaymentsApiSdk API uses an Access Token for authentication.
...
```ts
const sdk = new PaymentsApiSdk({ token: 'YOUR_TOKEN' });
```

If you need to set or update the access token after initializing the SDK, you can use:

```ts
const sdk = new PaymentsApiSdk();
...
```ts
const paymentsApiSdk = new PaymentsApiSdk({ timeout: 10000 });
```
